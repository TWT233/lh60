...
class RShiftRegionReportTest(unittest.TestCase):
    ...
    def test_report_has_the_complete_supported_region_schema(self):
        self.assertEqual(
            set(self.payload),
            {
                "schema_version",
        ...
        self.assertTrue(self.payload["production_pcb_modified"])
        self.assertFalse(self.payload["decision_required"])
        self.assertNotIn(
            "r3_rshift_2.75u",
    ...
    def test_single_g_search_is_exhaustive_and_reproducible(self):
        ...
        self.assertEqual(search["candidate_count"], 4**4)
        self.assertEqual(search["solved_candidate_count"], solved_candidate_count)
        self.assertGreater(solved_candidate_count, 0)
